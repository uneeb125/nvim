return {
    ...
    config = function()
        ...
        vim.keymap.set("n", "<leader>9s", function()
            _99.search()
        end, { desc = "Search project with prompt → quickfix list" })

        vim.keymap.set("n", "<leader>9v", function()
            _99.vibe()
        end, { desc = "Open-ended agentic vibe session" })

        vim.keymap.set("v", "<leader>9v", function()
            _99.visual()
        end, { desc = "Replace visual selection with AI" })

        vim.keymap.set("n", "<leader>9t", function()
            _99.tutorial()
        end, { desc = "Generate tutorial from prompt" })

        vim.keymap.set("n", "<leader>9o", function()
            _99.open()
        end, { desc = "Browse & open previous results" })

        vim.keymap.set("n", "<leader>9l", function()
            _99.view_logs()
        end, { desc = "View logs for past requests" })

        vim.keymap.set("n", "<leader>9x", function()
            _99.stop_all_requests()
        end, { desc = "Stop all in-flight requests" })

        vim.keymap.set("n", "<leader>9c", function()
            _99.clear_previous_requests()
        end, { desc = "Clear history of past operations" })

        vim.keymap.set("n", "<leader>9i", function()
            _99.info()
        end, { desc = "Show request count & loaded rules" })

        vim.keymap.set("n", "<leader>9m", function()
            require("99.extensions.telescope").select_model()
        end, { desc = "Select model via Telescope" })

        vim.keymap.set("n", "<leader>9p", function()
            require("99.extensions.telescope").select_provider()
        end, { desc = "Select provider via Telescope" })
    end,
}
